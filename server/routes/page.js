...
// NOTE: Literal paths must come BEFORE parameterized :pageId to avoid misrouting
router.get("/:boardId/pages", getPages);                      // List all pages
router.post("/:boardId/pages", createPage);                   // Create new page
router.patch("/:boardId/pages/reorder", reorderPages);        // Reorder pages (must be before :pageId)
